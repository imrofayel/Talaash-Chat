      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            <h3 className="font-medium text-lg text-gray-900 mb-1">{result.title}</h3>
            <a
              href={result.url}
              target="_blank"
          ...
          {result.text && (
            <div className="relative">
              <div className={cn("text-gray-700 overflow-hidden", !expanded && "max-h-24")}>
                <Markdown>{expanded ? result.text : `${result.text.slice(0, 200)}...`}</Markdown>
              </div>
          ...
          {hasSubpages && (
            <div className="mt-3">
              ...
              >
                {showSubpages ? "Hide" : "Show"} related pages ({result.subpages?.length})
                {showSubpages ? (
              ...
              {showSubpages && (
                <div className="mt-3 pl-4 border-l-2 border-gray-200">
                  {result.subpages?.map((subpage, idx) => (
                    <div key={idx} className="mb-4">
                      <div className="flex justify-between items-start">
                        <h4 className="font-medium text-gray-900">{subpage.title}</h4>
                        <a
                          href={subpage.url}
                          target="_blank"
                          rel="noopener noreferrer"
